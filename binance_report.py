#!/usr/bin/env python3
"""
Binance candle (kline) data puller + report generator.

For a given crypto symbol it pulls each candle from Binance's PUBLIC market-data
endpoint (no API key required) and produces a CSV report with, per candle:
...
series, then only the requested N candles are written out.
...
def print_summary(rows, symbol, interval):
    if not rows:
    ...
    print(f"\n{symbol.upper()}  |  interval {interval}  |  {n} candles")
    print(f"Period: {rows[0]['open_time']}  ->  {rows[-1]['open_time']} (UTC)")
    print(f"{'Window net change':<17} : {change_pct:.2f}%")
    print(f"{'Window range':<17} : {range_abs:.2f} ({range_pct:.2f}%)")
...
def main():
    ...
    # Fetch the requested window plus warm-up candles so every output row's MAs are filled.
    total = args.limit + WARMUP
    print(f"Fetching {args.limit} '{args.interval}' candles for {args.symbol.upper()} "
          f"(+{WARMUP} warm-up) ...")
    try:
        raw = fetch_klines(args.symbol, args.interval, total)
        ...
        sys.exit(f"Network error: {e}\nIf api.binance.com is blocked in your region, "
                 f"switch BASE_URL to the data-api.binance.vision mirror near the top of the file.")

    rows = build_rows(raw)
    add_moving_averages(rows)

    # Output only the N candles the user asked for (MAs computed over the full series).
    rows = rows[-args.limit:]

    write_csv(rows, output)
    print_summary(rows, args.symbol, args.interval)
